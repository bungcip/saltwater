...
impl<I: Lexer> Parser<I> {
    #[inline]
    pub(crate) fn expr(&mut self) -> SyntaxResult<Expr> {
        let start = self.unary_expr()?;
        self.binary_expr(start, 0)
    }
    #[inline]
    pub(crate) fn assignment_expr(&mut self) -> SyntaxResult<Expr> {
        self.custom_expr(BinaryPrecedence::Assignment(AssignmentToken::Equal))
    }
    #[inline]
    pub(crate) fn ternary_expr(&mut self) -> SyntaxResult<Expr> {
