...
fn parse_header_name(tokens: &mut std::slice::Iter<Token>) -> Option<(IncludeStyle, String)> {
    tokens.try_eat(|tokens| match tokens.next()? {
        Token::Punct('<') => {
            let mut name = String::new();
            for tok in tokens {
                if let Token::Punct('>') = tok {
        ...
        Token::Literal(s) if s.starts_with('"') && s.ends_with('"') => {
...
impl<'a> Macro<'a> {
    fn parse(mut tokens: std::slice::Iter<'a, Token>) -> Self {
        let params = tokens.try_eat(|tokens| {
            if !tokens.eat(&Token::Punct('(')) {
                return None;
            }
            tokens.eat(&Token::Whitespace);

            let mut params = IndexSet::new();
            while let Some(Token::Ident(param)) = tokens.eat_if(|tok| matches!(tok, Token::Ident(_))) {
                tokens.eat(&Token::Whitespace);
            ...
            let is_variadic = tokens.eat(&Token::Punct('.')) && {
        ...
        loop {
            ...
            let param = tokens.try_eat(|tokens| {
                let mode = if tokens.eat(&Token::Punct('#')) {
                    tokens.eat(&Token::Whitespace);
                ...
                match tokens.next()? {
                    Token::Ident(name) => Some((mode, param_idx(name)?)),
                    _ => None,
                }
            });
            ...
            let replacement = match concat_lhs {
                Some(mut parts) => {
                    let rhs = match param {
                        ...
                        None => tokens.next().map(ConcatPart::Token),
                    };
                    parts.extend(rhs);
...
impl<'a> Expander<'a> {
    ...
    pub fn with_defines_from(src: &'a SourceFile, phase4: &Expander<'a>) -> Self {
        Expander {
            enclosing_src_file: src,
            enclosing_header: None,
...
impl Macro<'_> {
    // FIXME(eddyb) consider changing code like this to take a `&mut Vec<Token>`
    fn try_expand(
        &self,
        name: &Ident,
        phase4: &Expander<'_>,
        outer: &OuterExpansion<'_>,
        args_tokens: &mut std::slice::Iter<'_, Token>,
    ) -> Option<Vec<Token>> {
        ...
        let args = if let Some((param_count, is_variadic)) = self.params {
            ...
            while tokens.eat(&Token::Whitespace) || tokens.eat(&Token::Newline) {}
            if !tokens.eat(&Token::Punct('(')) {
                return None;
            }
            ...
            loop {
                ...
                loop {
                    match tokens.next()? {
                        ...
                        Token::Punct(',') if is_variadic && args.len() == param_count => {}
                        Token::Punct(',') => break,
        ...
        let stringify_arg = |i: usize| {
            ...
            for tok in args[i] {
                ...
                if let Token::Whitespace | Token::Newline = tok {
            ...
            MacroBody::CondOnly(special) => {
                ...
                let value = match special {
                    ...
                    SpecialCondMacro::HasBuiltin => match args[0] {
                        ...
                        [Token::Ident(builtin)] => builtin.starts_with("__"),
                    ...
                    SpecialCondMacro::IsIdentifier => match args[0] {
                        ...
                        [Token::Ident(ident)] => !ident.starts_with("__"),

                        _ => return None,
                    },
                };
                return Some(vec![Token::Literal((value as u8).to_string())]);
            }
                        ...
                        for &part in parts.iter().skip(1) {
                            ...
                            tok = match (tok, rhs) {
                                ...
                                (Some(lhs), Some(rhs)) => match (lhs, rhs) {
                                    ...
                                    (Token::Ident(lhs), Token::Ident(rhs)) => Some(Token::Ident(Ident {
                                    ...
                                    (lhs, rhs) => {
                                        ...
                                        for tok in Tokenizer::new(&concatenated) {
...
impl Expander<'_> {
    // FIXME(eddyb) consider changing code like this to take a `&mut Vec<Token>`
    fn expand_macros(&self, tokens: &[Token], outer: &OuterExpansion<'_>) -> Vec<Token> {
                ...
                if name == "defined" && outer.originates_from_cond()
                    && let Some(arg_name) = tokens.try_eat(|tokens| {
                        ...
                        match tokens.next()? {
                            ...
                            Token::Punct('(') => {
                                tokens.eat(&Token::Whitespace);
                                if let Token::Ident(name) = tokens.next()? {
                                    tokens.eat(&Token::Whitespace);
                                    if tokens.eat(&Token::Punct(')')) {
                                        return Some(name);
                                    }
                        ...
                        output_tokens.push(Token::Literal((is_defined as u8).to_string()));
                        any_expansions = true;
                        continue;
                    }
...
impl<'a> CondEval<'a> {
    fn eat_op(&mut self, op: char) -> bool {
        self.tokens.eat(&Token::Whitespace);
        self.tokens.eat(&Token::Punct(op))
    }
    fn eat_op2(&mut self, op1: char, op2: char) -> bool {
        self.tokens.eat(&Token::Whitespace);
        self.tokens
            .eat_seq(iter::once(&Token::Punct(op1)).chain(iter::once(&Token::Punct(op2))))
    }
...
impl<'a> Expander<'a> {
    pub fn expand(&mut self) -> Vec<Token> {
        self.expand_group(&self.enclosing_src_file.phase3_group)
    }

    fn expand_group(&mut self, group: &'a Group) -> Vec<Token> {
            ...
            match part {
                ...
                GroupPart::Directive { maybe_name, tokens } => {
                    ...
                    if let Some(name) = maybe_name {
                        output_tokens.push(Token::Ident(name.clone()));
                    }
                    if !tokens.is_empty() {
                        output_tokens.push(Token::Whitespace);
                    }
